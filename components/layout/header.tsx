    ...
    <>
      ...
      <header className={`flex gap-2 sticky top-0 z-40 border-b border-black/10 bg-white/95 backdrop-blur p-2 ${isHomePage ? " pl-4" : ""} md:static md:bg-white`}>
          ...
          <form action="/search" method="get" className="flex w-full h-10 justify-between items-center rounded-md border border-[var(--color-primary)] bg-white pl-2 xl:flex">
            <input type="search" name="q" placeholder="Search for products..." autoComplete="off" aria-label="Search products" className="w-full !border-none bg-transparent text-sm text-[#C1C1C1] outline-none placeholder:text-[#8f9297]" />
